...
@dataclass
class Sale:
    """Represents a single property sale and its commission."""
    ...
    def _normalize_rate(rate: float) -> float:
        """Convert percentage values given in base 100 to decimal form when needed."""
    ...
    def revenue(self) -> float:
        """Commission amount earned from the sale."""

        if self.commission_override is not None:
            return self.commission_override
        return self.property_value * self.commission_rate

    @revenue.setter
    def revenue(self, value: float) -> None:
        """Allow overriding the commission amount directly."""

        self.commission_override = float(value)
    ...
    def from_dict(cls, payload: MutableMapping[str, object]) -> "Sale":
        ...
        if "property_value" in payload and "commission_rate" in payload:
            property_value = float(payload["property_value"])
            commission_rate = cls._normalize_rate(float(payload["commission_rate"]))
            commission_override: Optional[float]
            if "commission_override" in payload:
                commission_override = float(payload["commission_override"])
            elif "commission_amount" in payload:
                commission_override = float(payload["commission_amount"])
            else:
                commission_override = None
        elif "revenue" in payload:
            # Fallback to treat provided revenue as the commission amount.
            commission_amount = float(payload["revenue"])
            property_value = commission_amount
            commission_rate = 1.0
            commission_override = commission_amount
        else:
            raise KeyError(
                "Sale definition must include 'property_value' and 'commission_rate' or a direct 'revenue'."
            )
        ...
        return Scenario(
            ...
            sales=[
                Sale(
                    ...
                    dict(sale.variable_costs),
                    sale.commission_override,
                    sale.notes,
                )
                for sale in self.sales
            ],
    ...
    def apply_global_scaling(
        ...
        *,
        ...
        if revenue_multiplier is not None:
            for sale in self.sales:
                sale.property_value *= revenue_multiplier
                if sale.commission_override is not None:
                    sale.commission_override *= revenue_multiplier
        ...
        for sale in self.sales:
            if sale.id == sale_id:
                if field == "revenue":
                    ...
                    sale.property_value = value
                elif field in {"commission_rate", "commission_percentage"}:
                    sale.commission_rate = Sale._normalize_rate(value)
                elif field == "commission_override":
                    sale.commission_override = value
