...
class Sale:
    ...
    def from_dict(cls, payload: MutableMapping[str, object]) -> "Sale":
        ...
        elif "revenue" in payload and "sale_value" in payload:
            revenue_value = float(payload["revenue"])
            commission_rate = revenue_value / sale_value if sale_value else 0.0
        else:
            raise KeyError(
                "Sale entry with 'sale_value' must also include 'commission_rate' or explicit 'revenue'"
            )
        sale = cls(
            id=str(payload["id"]),
