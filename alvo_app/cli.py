...
def apply_overrides(scenario: Scenario, overrides: Iterable[str]) -> None:
    for override in overrides:
        ...
        if key.startswith("fixed."):
            ...
            if len(parts) != 3:
                raise ValueError(
                    "Use o formato sale.ID.campo=valor. Exemplo: sale.V001.property_value=1000000"
                )
            ...
            raise ValueError(
                ...
                "fixed.aluguel=12000 ou sale.V001.commission_rate=5. "
...
def apply_scaling(scenario: Scenario, args: argparse.Namespace) -> None:
    ...
    for broker_scale in args.scale_broker or []:
        broker, field, raw_multiplier = parse_three_part_entry(
            broker_scale,
            (
                "Use o formato 'NomeDoCorretor.campo=valor'. "
                "Campos suportados: revenue (ajusta o valor do imóvel) ou variable_costs"
            ),
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--scale-revenue",
        type=float,
        help="Multiplicador para o valor dos imóveis (impacta a comissão recebida)",
    )
    parser.add_argument(
            ...
            "sales": [
                {
                    "id": sale.id,
                    "broker": sale.broker,
                    "property_value": sale.property_value,
                    "commission_rate": sale.commission_rate,
                    "variable_costs": sale.variable_costs,
                    **(
                        {"commission_override": sale.commission_override}
